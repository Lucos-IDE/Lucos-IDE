...
async function main(buildDir?: string) {
	...
	const x64AppPath = path.join(buildDir, 'Lucos-darwin-x64', appName);
	const arm64AppPath = path.join(buildDir, 'Lucos-darwin-arm64', appName);
	const asarRelativePath = path.join('Contents', 'Resources', 'app', 'node_modules.asar');
	const outAppPath = path.join(buildDir, `Lucos-darwin-${arch}`, appName);
